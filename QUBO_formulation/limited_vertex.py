...
def reduce_graph(G, lower_bound):
  ...
  common_neighbours = np.logical_and(G, v_neighbours_mask).sum(axis=1, keepdims=True).reshape(-1)
...
def split(G, vertex_limit, lower_bound, solver):
  ...
  while len(subgraphs) > 0 and -subgraphs[0][0] > vertex_limit:
    _, _, sg, sg_indices, sg_pending_indices = heapq.heappop(subgraphs)
    ...
    if sg_indices.shape[0] > 0:
      if sg_indices.shape[0] <= vertex_limit:
        ...
        if combined_clique_size > lower_bound:
          ...
          max_clique_indices = set(sg_indices[sg_solution_mask]).union(sg_pending_indices)

      else:
        ind += 1
        heapq.heappush(subgraphs, (-sg.shape[0], ind, sg, sg_indices, sg_pending_indices))
    ...
    if ssg_indices.shape[0] > 0:
      if ssg_indices.shape[0] <= vertex_limit:
        ...
        if combined_clique_size > lower_bound:
          lower_bound = combined_clique_size
          max_clique_indices = set(ssg_indices[ssg_solution_mask]).union(sg_pending_indices, {v_ind})
